...
class Louvain {
 public:
  ...
  void vector_init_partition(vector<int> &new_n2c);
  void vector_init_partition2(vector<int> &new_n2c);
  
  void file_init_partition(char *filename, vector<int> &first_renum, vector<int> &proj_renum);
  ...
  BinaryGraph partition2graph_binary(vector<long double> &q);
  BinaryGraph partition2bipartitegraph_binary(vector<long double> &q, vector<long double> &d, vector<int> &n2c, vector<int> &new_node);

  // compute communities of the graph for one level
  // return true if some nodes have been moved
  bool one_level();
  
  //Do agglomerative clustering. Very slow!
  bool one_level_agg();
};
