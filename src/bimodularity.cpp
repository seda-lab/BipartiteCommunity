...
BiModularity::BiModularity(BinaryGraph & gr, vector<long double> &q_in):Quality(gr,"Bipartite Modularity") {
...
BiModularity::~BiModularity() {
  in.clear();
  tot.clear();
}

long double
BiModularity::quality() {
  long double qp  = 0.0L;
  long double m2 = g.total_weight;

  for (int i=0 ; i<size ; i++) {
    if (m2 > 0.0L){ 
		qp += in[i]/m2 - (tot[i]*tot[i]) / (F*F); 
	}
  }
