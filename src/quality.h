...
class Quality {
 public:
  ...
  BinaryGraph & g; // network to compute communities for
  int size; // nummber of nodes in the network and size of all vectors
  string name;
  int Nleft, Nright; //bipartite sizes
  
  vector< long double > qin; //left degrees
  vector< long double > din; //right degrees
  
  vector<int> n2c; // community to which each node belongs
  vector< vector<int> > c2n; // nodes in each communtiy
  Quality(BinaryGraph &gr, const std::string& n):g(gr),size(g.nb_nodes),name(n),Nleft(0),Nright(0){ qin.resize(0); din.resize(0); }
  ...
  // compute the gain of quality by adding comm to comm
  virtual long double agg_gain(int comm1, int comm2, long double v_12)=0;

  // compute the add comm to comm
  virtual void agg(int comm1, int comm2, long double v_12)=0;
